...
video = mmread('flame.avi',1:T); %extracted first T frames
data = zeros(H,W,T);
for i = 1:T
...
saveas(gcf,sprintf('flames_%d_coded_snapshot.png',T));  %Coded Snapshot
pause(3);
close(gcf);

dct_0 = dctmtx(8);
dct_1 = kron(dct_0,dct_0);
dct = blkdiag(dct_1,dct_1,dct_1,dct_1,dct_1)';     %psi - 2D DCT matrix
...
for k = 1:T
    ...
    saveas(gcf,sprintf('flames_%d_%d.png',T,k));
    pause(3);
    close(gcf);
end
